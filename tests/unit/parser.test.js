...
const { parseProperty, parseProperties } = require('../../lib/parser');
...
describe('parser', () => {
  ...
  describe('parseProperty', () => {
    it('should parse property with all data available', () => {
      // Mock title element
      const mockTitleElement = {
        text: jest.fn().mockReturnValue('Test Property Title'),
      ...
      const result = parseProperty(mockElement, mockCheerio);
    ...
    it('should handle relative links by making them absolute', () => {
      const mockTitleElement = {
        text: jest.fn().mockReturnValue('Test Property'),
  ...
  describe('parseProperties', () => {
    it('should parse multiple properties from HTML and filter valid ones', () => {
      // Test with realistic HTML that would actually produce some valid and invalid results
      const realCheerio = require('cheerio');
      
      // Create HTML with one valid property and one invalid (no title)
      const mockHTML = `
        <div class="item">
          <div class="item-info-title"><a href="/12345">Valid Property Title</a></div>
      ...
      const result = parseProperties(mockHTML, realCheerio);

      // Should only return the valid property (the one with a title)
    ...
    it('should return empty array when no valid properties found', () => {
      const realCheerio = require('cheerio');
