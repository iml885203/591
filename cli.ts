...
import { getUrlStationInfo } from './lib/multiStationCrawler';
...
type NotifyMode = 'all' | 'filtered' | 'none';
type FilteredMode = 'normal' | 'silent' | 'none';

// Main execution
if (require.main === module) {
  const args = process.argv.slice(2);
  const url = args[0];
  let maxLatest: number | null = null;
  let notifyMode: NotifyMode = 'filtered';
  let filteredMode: FilteredMode = 'silent';
  let multiStationOptions: MultiStationOptions = {};
  
  // Parse arguments
  for (let i = 1; i < args.length; i++) {
    if (args[i].startsWith('--notify-mode=')) {
      notifyMode = args[i].split('=')[1] as NotifyMode;
    } else if (args[i].startsWith('--filtered-mode=')) {
      filteredMode = args[i].split('=')[1] as FilteredMode;
  ...
  if (!url) {
    console.error('Usage: bun cli.ts <591_url> [max_latest] [--notify-mode=MODE] [--filtered-mode=FILTERED_MODE]');
    console.error('');
    ...
    console.error('  bun cli.ts "URL" 5                              # Latest 5, filtered+silent');
    console.error('  bun cli.ts "URL" --notify-mode=all              # All rentals, normal notifications');
    console.error('  bun cli.ts "URL" --filtered-mode=none           # Skip far rentals');
    console.error('  bun cli.ts "URL" --notify-mode=filtered --filtered-mode=normal  # All rentals, normal notifications');
    process.exit(1);
  }
  ...
  crawlWithNotifications(url, maxLatest, { 
    ...
    .then((result) => {
      ...
      if (summary.multiStation) {
        logger.info(`Multi-station crawl: ${summary.stationCount} stations (${summary.stations?.join(', ') || 'unknown'})`);
