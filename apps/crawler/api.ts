...
import { crawlWithNotifications } from './lib/crawlService';
...
import QueryId from './lib/domain/QueryId';
import SearchUrl from './lib/domain/SearchUrl';
import UrlNormalizer from './lib/domain/UrlNormalizer';
import fs from 'fs';
...
interface CrawlRequest {
  url: string;
  maxLatest?: number;
  notifyMode?: 'all' | 'filtered' | 'none';
  filteredMode?: 'normal' | 'silent' | 'none';
  filter?: {
...
const authenticateApiKey = (req: AuthenticatedRequest, res: Response, next: NextFunction): void => {
...
app.post('/crawl', authenticateApiKey, async (req: Request<{}, ApiResponse, CrawlRequest>, res: Response<ApiResponse>) => {
 ...
 *       - ApiKeyAuth: []
 */
app.post('/query/parse', authenticateApiKey, async (req: Request<{}, ApiResponse, { url: string }>, res: Response<ApiResponse>) => {
  try {
    const { url } = req.body;
 ...
 */
app.delete('/query/:queryId/clear', authenticateApiKey, async (req: AuthenticatedRequest, res: Response<ApiResponse>) => {
  ...
  try {
    ...
    const cleared = await databaseStorage.prisma.$transaction(async (tx: any) => {
      ...
      const rentalIds = queryRentals.map((qr: any) => qr.rentalId);
      ...
      if (rentalIds.length > 0) {
        ...
        const orphanedRentalIds = orphanedRentals.map((r: any) => r.id);
 ...
 *       - ApiKeyAuth: []
 */
app.get('/query/:queryId/rentals', authenticateApiKey, async (req: AuthenticatedRequest, res: Response<ApiResponse>) => {
  try {
    const { queryId } = req.params;
 ...
 *       - ApiKeyAuth: []
 */
app.get('/queries', authenticateApiKey, async (req: AuthenticatedRequest, res: Response<ApiResponse>) => {
  try {
 ...
 *       - ApiKeyAuth: []
 */
app.get('/query/:queryId/similar', authenticateApiKey, async (req: AuthenticatedRequest, res: Response<ApiResponse>) => {
  try {
    const { queryId } = req.params;
 ...
 *       - ApiKeyAuth: []
 */
app.get('/query/statistics', authenticateApiKey, async (req: Request, res: Response<ApiResponse>) => {
  try {
    const databaseStorage = new DatabaseStorage();
    await databaseStorage.initialize();
 ...
 *       - ApiKeyAuth: []
 */
app.get('/debug/html', authenticateApiKey, async (req: Request, res: Response<ApiResponse>) => {
  try {
 ...
 *       - ApiKeyAuth: []
 */
app.get('/debug/html/:filename', authenticateApiKey, async (req: Request<{ filename: string }>, res: Response) => {
  try {
...
if (require.main === module) {
  ...
  const checkDatabaseOnStartup = async (): Promise<void> => {
