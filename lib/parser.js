...
const parseRental = ($el, $) => {
  ...
  if (Math.random() < 0.1 || rooms === '房型未明' || !rooms) { // Log issues + random sample
    console.log(`[PARSER DEBUG] Title: "${title}"`);
    console.log(`[PARSER DEBUG] Raw houseType: "${houseType}" (type: ${typeof houseType})`);
    console.log(`[PARSER DEBUG] Raw rooms: "${rooms}" (type: ${typeof rooms})`);
    
    // Additional debug for rooms parsing
    console.log(`[PARSER DEBUG] houseHomeSection found: ${houseHomeSection.length > 0}`);
    if (houseHomeSection.length > 0) {
      const allSpans = houseHomeSection.find('span');
      console.log(`[PARSER DEBUG] Total spans in section: ${allSpans.length}`);
      allSpans.each((i, span) => {
        const $span = houseHomeSection.find(span);
        const className = $span.attr('class') || 'no-class';
        const text = $span.text().trim();
        console.log(`[PARSER DEBUG]   span ${i}: class="${className}", text="${text}"`);
      });
    }
  }
  ...
  // Enhanced room parsing - prioritize title parsing for reliability
  const titleRoomMatch = title.match(/(\d+房\d*廳?\d*衛?|套房|雅房|分租套房|\d+房|\d+廳)/);
  
  if (titleRoomMatch) {
    // Title has room info - use it (more reliable)
    rooms = titleRoomMatch[1];
  } else if (!rooms || rooms.length === 0 || rooms.includes('?')) {
    // No room info found anywhere
    rooms = '房型未明';
  }
  // else: keep HTML-parsed rooms if title doesn't have info
  ...
  if (Math.random() < 0.1) { // Log ~10% of rentals to avoid spam
    console.log(`[PARSER DEBUG] Final result - houseType: "${result.houseType}", rooms: "${result.rooms}"`);
  }
