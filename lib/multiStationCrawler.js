...
const crawlMultipleStations = async (originalUrl, options = {}, dependencies = {}) => {
  ...
  logWithTimestamp(`Starting multi-station crawl for: ${originalUrl}`);
  ...
  logWithTimestamp(`Found ${stationUrls.length} station URLs to crawl`);
  
  if (stationUrls.length === 1) {
    // Single station, use normal crawling
    const rentals = await crawl591(originalUrl, dependencies);
    return {
      rentals,
      stationCount: 1,
      stations: stations,
  ...
  if (mergeResults) {
    ...
    logWithTimestamp(`Crawling completed. Found ${mergedResults.rentals.length} unique rentals from ${stationUrls.length} stations`);
 ...
 * @private
 */
const crawlStationsWithConcurrency = async (stationUrls, stations, maxConcurrent, delay, dependencies) => {
  const results = [];
  ...
  const crawlPromises = stationUrls.map(async (searchUrl, index) => {
    return semaphore(async () => {
      ...
      try {
        logWithTimestamp(`Crawling station ${stationId}: ${url}`);
        ...
        const rentals = await crawl591(url, dependencies);
        
        logWithTimestamp(`Station ${stationId} completed: ${rentals.length} rentals found`);
        ...
        logWithTimestamp(`Station ${stationId} failed: ${error.message}`, 'ERROR');
        return {
          stationId,
          url,
          rentals: [],
