/**
 * Crawl Service - Main application service orchestrating crawling workflow
 * Application Service Layer - Coordinates between domain, infrastructure, and external services
 * 
 * Responsibilities:
 * - Orchestrate crawling workflow
 * - Coordinate between domain objects and infrastructure services
 * - Handle cross-cutting concerns (notifications, storage)
...
const crawlWithNotificationsMultiStation = async (url, maxLatest = null, options = {}, dependencies = {}) => {
  ...
  try {
    ...
    logger.error(`Multi-station crawl service error: ${error.message}`);
