...
import { QueryId } from '../domain/QueryId';
...
interface CrawlOptions {
  ...
  multiStationOptions?: Record<string, any>;
  newRentals?: number;
...
interface QueryData {
  ...
  totalRentals: number; // alias for test compatibility
  crawlSessions: any[];
  rentals: any[];
}
...
export class DatabaseStorage {
   ...
   */
  async saveCrawlResults(url: string, rentals: RentalData[], crawlOptions: CrawlOptions = {}): Promise<SavedCrawlInfo | null> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error(`Error saving crawl results: ${errorMessage}`);
      throw error;
    }
  }
   ...
   * @private
   */
  private async _upsertRental(tx: TransactionClient, rentalData: RentalData, sessionId: string, queryId: string): Promise<any> {
    ...
    if (!comparison.hasChanged && existingRental) {
      ...
      where: { propertyId },
        update: {
          ...
          tags: (rentalData.tags as string[])?.join(',') || null,
          imgUrls: (rentalData.imgUrls as string[])?.join(',') || null,
      ...
      if (existingRental && comparison.changedFields.length > 0) {
        comparison.changedFields.forEach(field => {
          if (field !== 'new_record') {
            const oldValue = (existingRental as any)[field] || '';
            const newValue = (rentalData as any)[field] || '';
    ...
    if (comparison.hasChanged || !existingRental) {
      const metroDistanceOperations: Promise<any>[] = [];
   ...
   * @returns Set of existing property IDs
   */
  async getExistingPropertyIds(queryId: string): Promise<Set<string>> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error(`Error getting existing property IDs: ${errorMessage}`);
      return new Set();
    }
  }
   ...
   */
  async getRentalsForQuery(queryId: string, options: QueryOptions = {}): Promise<QueryData | null> {
  ...
  async getQueryRentals(queryId: string, options: QueryOptions = {}): Promise<QueryData | null> {
    try {
      ...
      if (query.rentals && query.rentals.length > 0) {
        ...
        sampleRentals.forEach((qr: any, index: number) => {
          if (qr.rental) {
      ...
      return {
        ...
        rentals: query.rentals.map((qr: any) => ({
          ...qr.rental,
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error(`Error getting query rentals: ${errorMessage}`);
      throw error;
    }
  }
  ...
  async listQueries(options: QueryListOptions = {}): Promise<QueriesListResult> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error(`Error listing queries: ${errorMessage}`);
      throw error;
    }
  }
  ...
  async getStatistics(): Promise<StorageStatistics> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error(`Error getting statistics: ${errorMessage}`);
      throw error;
    }
  }
   ...
   */
  async findSimilarQueries(queryId: string, options: { limit?: number } = {}): Promise<SimilarQuery[]> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error(`Error finding similar queries: ${errorMessage}`);
      throw error;
    }
  }
  ...
  async deleteQuery(queryId: string): Promise<boolean> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error(`Error deleting query: ${errorMessage}`);
      throw error;
    }
  }
   ...
   */
  async batchUpsertRentalsOptimized(rentals: RentalData[], queryId: string, options: any = {}): Promise<any[]> {
   ...
   * @private
   */
  private _calculateSimilarity(query1: any, query2: any): number {
    let score = 0;
