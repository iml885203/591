...
class DatabaseOptimizer {
  ...
  async cachedQuery(cacheKey, queryFn, ttlMs = this.cacheConfig.ttlMs) {
    ...
    // Log slow queries
    if (duration > 1000) {
      logWithTimestamp(`🐌 Slow query detected: ${cacheKey} took ${duration}ms`, 'WARN');
    }
  ...
  async getExistingPropertyIdsOptimized(queryId) {
    ...
    return this.cachedQuery(cacheKey, async () => {
      ...
      const duration = Date.now() - startTime;
      logWithTimestamp(`📊 Fetched ${totalFetched} existing property IDs in ${duration}ms`);
  ...
  async batchUpsertRentalsOptimized(rentals, queryId, options = {}) {
    ...
    logWithTimestamp(`🔄 Starting optimized batch upsert: ${rentals.length} rentals`);
    ...
    for (let i = 0; i < rentals.length; i += batchSize) {
      ...
      try {
        ...
        logWithTimestamp(`✅ Batch ${Math.floor(i/batchSize) + 1} completed: ${batch.length} rentals in ${batchDuration}ms`);

      } catch (error) {
        logWithTimestamp(`❌ Batch ${Math.floor(i/batchSize) + 1} failed: ${error.message}`, 'ERROR');
        
        // Try individual upserts for failed batch
        logWithTimestamp(`🔄 Retrying batch individually...`);
        for (const rentalData of batch) {
          try {
            ...
            logWithTimestamp(`❌ Individual upsert failed for ${rentalData.title}: ${individualError.message}`, 'ERROR');
          }
        }
      }
    }
  ...
  _cleanupCache() {
    ...
    if (toDelete.length > 0) {
      logWithTimestamp(`🧹 Cleaned ${toDelete.length} expired cache entries`);
    }
  }
  ...
  clearCache() {
    ...
    logWithTimestamp(`🧹 Cleared query cache: ${oldSize} entries removed`);
  }
  ...
  async getPerformanceMetrics() {
    try {
      ...
      logWithTimestamp(`Error getting performance metrics: ${error.message}`, 'ERROR');
      return null;
    }
  }
