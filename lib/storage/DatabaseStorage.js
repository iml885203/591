...
class DatabaseStorage {
  constructor() {
    ...
    logWithTimestamp('🚀 Database optimizer initialized');
  }
  ...
  async saveCrawlResults(url, rentals, crawlOptions = {}) {
    try {
      ...
      const result = await this.prisma.$transaction(async (tx) => {
        ...
        const batchSize = 5; // Reduce batch size to 5 rentals at a time to avoid timeout
        ...
        for (let i = 0; i < rentals.length; i += batchSize) {
          ...
          if (rentals.length > 20) {
            logWithTimestamp(`Processed ${Math.min(i + batchSize, rentals.length)}/${rentals.length} rentals`);
          }
        }
        ...
        maxWait: 45000, // Maximum time to wait for a transaction slot (increased)
        timeout: 120000, // Maximum time allowed for the transaction (2 minutes)
      });
  ...
  async _upsertRental(tx, rentalData, sessionId, queryId) {
    ...
    // DEBUG LOG: Check data before database upsert
    if (Math.random() < 0.2) { // Log ~20% of rentals
      console.log(`[DB UPSERT DEBUG] Property: ${propertyId}`);
      console.log(`[DB UPSERT DEBUG] Input houseType: "${rentalData.houseType}" (type: ${typeof rentalData.houseType})`);
      console.log(`[DB UPSERT DEBUG] Input rooms: "${rentalData.rooms}" (type: ${typeof rentalData.rooms})`);
      console.log(`[DB UPSERT DEBUG] Will save houseType: "${rentalData.houseType || '房屋類型未明'}"`);
      console.log(`[DB UPSERT DEBUG] Will save rooms: "${rentalData.rooms || '房型未明'}"`);
    }
    ...
    // Handle metro distances - batch process to avoid individual upsert overhead
  ...
  async getExistingPropertyIds(queryId) {
    try {
      ...
      logWithTimestamp(`Error getting existing property IDs: ${error.message}`, 'ERROR');
      return new Set();
    }
  }
  ...
  async getQueryRentals(queryId, options = {}) {
    try {
      ...
      // DEBUG LOG: Check data being returned by API
      if (query.rentals && query.rentals.length > 0) {
        const sampleRentals = query.rentals.slice(0, 3);
        console.log(`[API RETURN DEBUG] Returning ${query.rentals.length} rentals for query ${queryId}`);
        sampleRentals.forEach((qr, index) => {
          if (qr.rental) {
            console.log(`[API RETURN DEBUG] Rental ${index + 1}:`);
            console.log(`[API RETURN DEBUG]   Title: "${qr.rental.title}"`);
            console.log(`[API RETURN DEBUG]   houseType: "${qr.rental.houseType}" (type: ${typeof qr.rental.houseType})`);
            console.log(`[API RETURN DEBUG]   rooms: "${qr.rental.rooms}" (type: ${typeof qr.rental.rooms})`);
          }
        });
      }
