...
class DatabaseStorage {
  ...
  async _upsertRental(tx, rentalData, sessionId, queryId) {
    ...
    // DEBUG LOG: Check data before database upsert (controlled by DEBUG_LOGS)
    const { debugLog } = require('../logger');
    if (Math.random() < 0.2) { // Log ~20% of rentals
      debugLog('DB_UPSERT', `Property: ${propertyId}`);
      debugLog('DB_UPSERT', `Input houseType: "${rentalData.houseType}" (type: ${typeof rentalData.houseType})`);
      debugLog('DB_UPSERT', `Input rooms: "${rentalData.rooms}" (type: ${typeof rentalData.rooms})`);
      debugLog('DB_UPSERT', `Will save houseType: "${rentalData.houseType || '房屋類型未明'}"`);
      debugLog('DB_UPSERT', `Will save rooms: "${rentalData.rooms || '房型未明'}"`);
    }
