...
function autoConfigureDatabase() {
  ...
  if (process.env.DATABASE_URL) {
    logWithTimestamp('Using PostgreSQL database from DATABASE_URL');
...
async function checkDatabaseConnection(prismaClient) {
  try {
    ...
    logWithTimestamp(`PostgreSQL connection check failed: ${error.message}`, 'ERROR');
    return false;
  }
}
...
async function initializeDatabase(prismaClient) {
  try {
    ...
    logWithTimestamp(`PostgreSQL database initialized - found ${queryCount} queries`);
  } catch (error) {
    if (error.code === 'P2021' || error.message.includes('does not exist')) {
      logWithTimestamp('PostgreSQL database not initialized - tables may need to be created');
