 ...
 */
const createPropertyEmbed = (property, index, total, silent = false, distanceThreshold = 800, originalUrl = '') => {
  ...
  const embed = {
    ...
    footer: { 
      text: `${index}/${total} - 591房源通知${isFarFromMRT ? ` (距離捷運>${distanceThreshold}m)` : ''}${silent ? ' 🔇' : ''}${originalUrl ? ` • ${originalUrl}` : ''}` 
    },
