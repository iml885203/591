...
interface MergedResults {
  ...
  errors: Array<{ stationId: string | null; error: string }>;
  successfulStations: number;
}

interface MultiStationCrawlResult {
  rentals?: any[];
  stationCount: number;
  stations: string[];
  mergedCount?: number;
...
interface SemaphoreTask<T> {
  (): Promise<T>;
}

interface QueueItem<T> {
  task: SemaphoreTask<T>;
 ...
 * @param originalUrl - Original URL with multiple stations
 * @param options - Crawling options
 * @param dependencies - Injected dependencies for testing
 * @returns Results with merged rentals and metadata
 */
export const crawlMultipleStations = async (
  originalUrl: string, 
 ...
 * @private
 */
const crawlStationsWithConcurrency = async (
  stationUrls: SearchUrl[], 
  stations: string[], 
  maxConcurrent: number, 
  delay: number, 
  crawlerFunction: typeof crawl591,
  dependencies: CrawlDependencies
): Promise<StationResult[]> => {
  const results: StationResult[] = [];
  const semaphore = createSemaphore<StationResult>(maxConcurrent);
  ...
  const crawlPromises = stationUrls.map(async (searchUrl, index) => {
    return semaphore(async () => {
      ...
      try {
        ...
        logger.error(`Station ${stationId} failed: ${errorMessage}`);
        return {
          stationId,
          url,
          rentals: [],
 ...
 * @private
 */
const mergeStationResults = (stationResults: StationResult[], includeStationInfo: boolean): MergedResults => {
  const rentalMap = new Map<string, Rental>(); // PropertyId -> Rental
  let totalFound = 0;
  let duplicateCount = 0;
  const errors: Array<{ stationId: string | null; error: string }> = [];

  stationResults.forEach(result => {
    if (!result.success) {
      errors.push({ stationId: result.stationId, error: result.error || 'Unknown error' });
      return;
    }
 ...
 * @private
 */
const createSemaphore = <T>(maxConcurrent: number) => {
  let running = 0;
  const queue: QueueItem<T>[] = [];

  return async (task: SemaphoreTask<T>): Promise<T> => {
 ...
 */
export const hasMultipleStations = (url: string): boolean => {
 ...
 */
export const getUrlStationInfo = (url: string) => {
  const searchUrl = new SearchUrl(url);
