...
const crawlWithNotifications = async (url, maxLatest = null, options = {}, dependencies = {}) => {
  ...
  try {
    ...
    } else if (candidateProperties.length > 0) {
      // Show details about the filtered properties
      logWithTimestamp(`Found ${candidateProperties.length} candidates but all filtered out by notification mode: ${notifyMode}${notifyMode === 'filtered' ? `/${filteredMode}` : ''}`);
      const sampleCount = Math.min(3, candidateProperties.length);
      logWithTimestamp(`Filtered properties (${sampleCount}): ${candidateProperties.slice(0, sampleCount).map(p => p.title).join(' | ')}`);
    } else {
