 ...
 */
const filterPropertiesForNotification = (properties, notifyMode, filteredMode, filter = {}) => {
...
const crawlWithNotifications = async (url, maxLatest = null, options = {}, dependencies = {}) => {
  ...
  try {
    ...
    const propertiesToNotify = notifyMode === 'none' ? [] : 
      filterPropertiesForNotification(candidateProperties, notifyMode, filteredMode, filter);
    
    // Add notification metadata to all properties for response
    const propertiesWithMetadata = addNotificationMetadata(allProperties, propertiesToNotify, { notifyMode, filteredMode, filter });
    ...
    if (candidateProperties.length > 0) {
      ...
      if (skippedCount > 0) {
        ...
        filteredProperties.forEach((property, index) => {
          ...
          logWithTimestamp(`✗ Filtered ${index + 1}/${filteredProperties.length}${distanceInfo}: ${property.title}`);
        });
      }
    } else {
    ...
    if (propertiesToNotify.length > 0) {
      ...
      await sendDiscordNotifications(propertiesToNotify, url, webhookUrl, axios, config, filter);
    }
