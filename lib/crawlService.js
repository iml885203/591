...
// File storage imports removed - now using database for tracking
const { sendDiscordNotifications, sendErrorNotification } = require('./notification');
const Rental = require('./Rental');
const DatabaseStorage = require('./storage/DatabaseStorage');

/**
 * Filter rentals to find new ones compared to previous crawl
 * @param {Array} currentRentals - Current rentals
 * @param {Set<string>} existingPropertyIds - Set of existing property IDs from database
 * @returns {Array} New rentals
 */
const findNewRentals = (currentRentals, existingPropertyIds) => {
 ...
 * @param {Object} databaseStorage - Database storage instance
 * @returns {Promise<Array>} Rentals to notify
 */
const getRentalsToNotify = async (rentals, maxLatest, url, databaseStorage) => {
  if (maxLatest) {
    ...
    if (!queryId || queryId === 'unknown') {
      logWithTimestamp('Cannot determine new rentals: invalid query ID', 'WARN');
      return [];
    }
    ...
    try {
      ...
      logWithTimestamp(`Error getting existing rentals from DB: ${error.message}`, 'ERROR');
    ...
    logWithTimestamp(`Existing rentals in DB: ${existingPropertyIds.size}`);
...
const crawlWithNotificationsMultiStation = async (url, maxLatest = null, options = {}, dependencies = {}) => {
  ...
  try {
    ...
    // DEBUG LOG: Check multi-station crawled rentals before saving
    if (allRentals && allRentals.length > 0) {
      const sampleRentals = allRentals.slice(0, 3);
      console.log(`[MULTI CRAWL DEBUG] About to save ${allRentals.length} multi-station crawled rentals`);
      sampleRentals.forEach((rental, index) => {
        console.log(`[MULTI CRAWL DEBUG] Rental ${index + 1}:`);
        console.log(`[MULTI CRAWL DEBUG]   Title: "${rental.title}"`);
        console.log(`[MULTI CRAWL DEBUG]   houseType: "${rental.houseType}" (type: ${typeof rental.houseType})`);
        console.log(`[MULTI CRAWL DEBUG]   rooms: "${rental.rooms}" (type: ${typeof rental.rooms})`);
      });
    }
